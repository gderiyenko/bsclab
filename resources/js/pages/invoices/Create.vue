                ...
                <form @submit.prevent="invoiceCreate">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="hidden" class="form-control"
                               v-model="invoice.firm_id = this.$route.params.firm_id">
...
export default {
            ...
            .then(response => {
                ...
                if (this.invoices.length) {
                    for (let key in this.invoices) {
                        if (Number(this.invoices[key]['number'])) {
                            this.invoicesNumbers.push(Number(this.invoices[key]['number']))
                        }
                    }
                    if (this.invoicesNumbers.length) {
                        this.maxNumberInvoice = this.invoicesNumbers.reduce(function (a, b) {
                            return Math.max(a, b);
                        });
                        this.invoice['number'] = ++this.maxNumberInvoice
                    } else {
    ...
    methods: {
        invoiceCreate() {
            this.axios
                .post(process.env.MIX_APP_URL + '/api/invoices/' + `${this.$route.params.firm_id}` + '/create', {
        ...
        getContracts() {
            this.axios
                .get(process.env.MIX_APP_URL + '/api/contracts/show/' + `${this.$route.params.firm_id}`)
                .then((response) => {
