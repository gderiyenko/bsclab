            ...
            <div class="row">
                <div class="col-sm-3">
                    <select class="form-control" v-model="tableFilter.status">
                        <option value="all">Діючі та не діючі</option>
                        <option value="active">Діючі</option>
                        <option value="notActive">Не діючі</option>
                    </select>
                </div>
                <div class="col-sm-6">
                    <h4 div class="text-center">Договори</h4>
                </div>
                <div class="col-sm-3 text-right">
                    ...
                    <date-range-picker
                        ...
                        <div slot="header" slot-scope="header" class="slot">
                            <h6><b>Інтервал дат договорів для фільтрації</b></h6>
                        </div>
                ...
                <template slot="table-row" slot-scope="props">
                    <span v-if="props.column.field === 'Назва контрагента (скорочена)'">
                        <router-link :to="{name: 'firms.show', params: { id: props.row.firm_id }}" :title="'Детально'">
                            {{ props.row.firm.firm_name_short }}
                        </router-link>
                    </span>
                </template>
...
export default {
    ...
    data() {
        ...
        return {
            ...
            json_fields: {
                '№ договору': 'number',
                'Дата договору': 'date_contract',
                'Дата припинення': 'date_end',
                'Контрагент': 'firm.firm_name_short',
                'Статус': 'status',
                'Сума, грн.': {
                    field: 'amount',
                    callback: (value) => {
            ...
            fileName: 'Перелік договорів',
        }
    },
    computed: {
        filterForContracts: function () {
    ...
    watch: {
        filterForContracts: {
            handler(val) {
                this.contractsForTable = this.contracts.filter(d => {
                    let time = new Date(d.date_contract).getTime();
                    return (this.tableFilter.dates.startDate < time && time < this.tableFilter.dates.endDate);
                });

                if (this.tableFilter.status == 'active') {
                    this.contractsForTable = this.contractsForTable.filter(function (contractsForTable) {
                        return contractsForTable.status == 'Діючий';
                    });
                }

                if (this.tableFilter.status == 'notActive') {
                    this.contractsForTable = this.contractsForTable.filter(function (contractsForTable) {
                        return contractsForTable.status == 'Не діючий';
                    });
                }
            },
            deep: true
